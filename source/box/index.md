---
title: 百宝箱
top_img: url("box.jpg")
toc: true
---
# 资源
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: 果核剥壳
      link: https://www.ghxi.com/
      avatar: https://www.ghxi.com/favicon.ico 
      descr: 互联网的净土
    - name: 亿破姐 
      link: https://www.ypojie.com/
      avatar: https://www.ypojie.com/favicon.ico 
      descr: 给你所需要的内容！
    - name: 异次元
      link: https://www.iplaysoft.com/
      avatar: https://www.iplaysoft.com/favicon.ico
      descr: 软件世界
    - name: 小森林导航
      link: https://www.xsldh.vip/
      avatar: https://www.xsldh.vip/theme/img/logo3.jpg
      descr: 娱乐网址大全
    - name: Jsons
      link: http://www.jsons.cn/
      avatar: http://www.jsons.cn/favicon.ico
      descr: 实用工具大全

{% endflink %}
# 创作
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: 智影
      link: https://zenvideo.qq.com/home
      avatar: https://material.gtimg.com/grail/dist/favicon.ico
      descr: 在线智能视频创作平台
    - name: 图说
      link: https://tushuo.baidu.com/
      avatar: https://tushuo.baidu.com/homepage/asset/img/echarts-logo.png
      descr: 图表制作
    - name: 速写板
      link: https://www.suxieban.com/
      avatar: https://www.suxieban.com/favicon.ico 
      descr: 在线画板
{% endflink %}
# 图片
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: Bigjpg
      link: https://bigjpg.com/
      avatar: https://bigjpg.com/favicon.ico 
      descr: 图片放大
    - name: Recompressor
      link: https://zh.recompressor.com/
      avatar: 	https://dinficfagt18c.cloudfront.net/p/assets/m/drawings/recompressor-favicon_8feb8450ef6aed0cf99733ca4016f77f.ico
      descr: 图片压缩
    - name: RemoveBG
      link: https://www.remove.bg/zh
      avatar:  https://www.remove.bg/favicon.ico
      descr: 背景消除
    - name: Photopea
      link: https://www.photopea.com/
      avatar: https://www.photopea.com/promo/thumb256.png
      descr: 在线Photoshop
{% endflink %}
# 音频
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: 小森平的免费下载音效
      link: https://taira-komori.jpn.org/freesoundcn.html
      avatar: https://taira-komori.jpn.org/favicon002000.ico 
      descr: 免费音效
    - name: LAlAL.AI
      link: https://www.lalal.ai/zh-hans/
      avatar: https://s.lalal.ai/icons/favicon.ico 
      descr: 人声/伴奏分离
{% endflink %}
# 视频
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: 短视频解析
      link: https://www.helloty.cn/
      avatar: https://www.helloty.cn/images/favicon.ico 
      descr: 去水印下载
    - name: Anime
      link: https://trace.moe/
      avatar: https://trace.moe/favicon128.png
      descr: 动漫片段识别
{% endflink %}
# 文件
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: ALL TO ALL
      link: https://www.alltoall.net/
      avatar: https://ccdn.goodq.top/caches/2bf25eeab1dd9ac68280087e9f26d920/aHR0cHM6Ly93d3cuYWxsdG9hbGwubmV0L3FmeS1jb250ZW50L3VwbG9hZHMvMjAxNi8wNi9mMjJlNjEzMGY2ZTJlZmM0YjIzYzBjMjc0MjNiMDM1OS5qcGc_p_p100_p_3D.jpg
      descr: 在线格式转换
    - name: LightPDF
      link: https://lightpdf.com/zh/
      avatar: https://qncdn.aoscdn.com/local/lightpdf.com/img/favicon.png
      descr: 在线PDF编辑器及转换器
{% endflink %}
# 游戏
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: 我的世界指令生成器
      link: https://mc.metamo.cn/gmct/
      avatar: https://mc.metamo.cn/favicon.ico 
      descr: Minecraft
    - name: 小霸王
      link: https://www.yikm.net/
      avatar: https://www.yikm.net/f.png
      descr: 各种年代感游戏集合
    - name: 奇趣网站收藏家
      link: https://fuun.fun/
      avatar: https://fuun.fun/favicon.ico
      descr: 游戏集合
    - name: A Dark Room(小黑屋)
      link: http://lab.mkblog.cn/adarkroom/
      avatar: http://lab.mkblog.cn/adarkroom/favicon.ico
      descr: 纯文字冒险游戏
    - name: 超苦逼冒险者
      link: http://kubitionadvanture.sinaapp.com/
      avatar: http://kubitionadvanture.sinaapp.com/favicon.ico
      descr: 纯文字冒险游戏
{% endflink %}
# 其它
{% flink %}
- class_name:
  class_desc: 
  link_list:
    - name: AutoPlan
      link: https://auto.oldwu.top/
      avatar: https://cdn.jsdelivr.net/gh/wyt1215819315/autoplan@master/src/main/resources/static/image/favicon.ico 
      descr: B站/网易云/米游社自动签到
    - name: 极简插件
      link: https://chrome.zzzmh.cn/
      avatar: https://chrome.zzzmh.cn/favicon.ico 
      descr: 浏览器插件下载
    - name: TTS
      link: http://nopapp.com/App/TTS
      avatar: http://nopapp.com/favicon.ico
      descr: 文字转语音
{% endflink %}
